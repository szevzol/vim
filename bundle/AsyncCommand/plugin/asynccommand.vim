...
command! -nargs=+ -complete=file AsyncFindGrep call s:AsyncFindGrep(<f-args>)
...
function! s:AsyncGrep(query)
    ...
    let grep_cmd = "grep ".a:query
    call asynccommand#run(grep_cmd, asynchandler#quickfix(&grepformat, '[Found: %s] grep -n ' . a:query))
endfunction

" Find + Grep
"   - open result in quickfix
function! s:AsyncFindGrep(directory, pattern, query)
    let find_grep_cmd = "find " . a:directory . " -name \"" . a:pattern . "\" | xargs grep -n ".a:query
    call asynccommand#run(find_grep_cmd, asynchandler#quickfix(&grepformat, '[Found: %s] grep ' . a:query))
endfunction
